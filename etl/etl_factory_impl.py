...
from postgres_load_command import PostgresLoadCommand
from no_command import NoCommand
from etl_factory import ETLFactory  

class ETLFactoryImpl(ETLFactory):
    def __init__(self, logger, dataset_path, output_format, output_path, conn_string=None, table_name=None):
        self.logger = logger
    ...
    def get_load(self):
        ...
        if self.output_format == "csv":
            return CSVLoadCommand(self.logger, self.output_path)
        elif self.output_format == "xlsx":
            return ExcelLoadCommand(self.logger, self.output_path)
        elif self.output_format == "json":
            return JsonLoadCommand(self.logger, self.output_path)
        elif self.output_format == "sql":
            if self.conn_string and self.table_name:
                return PostgresLoadCommand(self.conn_string, self.table_name, self.logger)
            else:
